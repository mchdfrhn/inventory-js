...
class AssetUseCase {
  ...
  async validateAssetDataForUpdate(assetData, isBulkUpdate = false) {
    // For updates, only validate fields that are provided

    // Validation rules for provided fields
    if (assetData.kode !== undefined) {
      if (!isBulkUpdate && (!assetData.kode || !assetData.kode.trim())) {
        throw new Error('Asset code (kode) cannot be empty if provided');
      }
      if (assetData.kode.length > 50) {
        throw new Error('Asset code must be 50 characters or less');
      }
    }

    if (assetData.nama !== undefined) {
      if (!assetData.nama || !assetData.nama.trim()) {
        throw new Error('Asset name (nama) cannot be empty if provided');
      }
      if (assetData.nama.length > 255) {
        throw new Error('Asset name must be 255 characters or less');
      }
    ...
    if (assetData.satuan !== undefined) {
      if (!assetData.satuan || !assetData.satuan.trim()) {
        throw new Error('Unit (satuan) cannot be empty if provided');
      }
      if (assetData.satuan.length > 50) {
        throw new Error('Unit must be 50 characters or less');
      }
    ...
    if (assetData.harga_perolehan !== undefined && assetData.harga_perolehan < 0) {
      throw new Error('Acquisition price must be greater than or equal to 0');
    }
    ...
    // Foreign key validation only if provided
    ...
    if (assetData.lokasi_id !== undefined) {
      const location = await this.locationRepository.getById(assetData.lokasi_id);
      if (!location) {
        throw new Error('Location not found');
      }
    ...
    // Clean up data
    if (assetData.kode !== undefined && !isBulkUpdate) {
      assetData.kode = assetData.kode.trim();
    }
    if (assetData.nama !== undefined) {
      assetData.nama = assetData.nama.trim();
    ...
    if (assetData.spesifikasi !== undefined) {
      assetData.spesifikasi = assetData.spesifikasi.trim();
    }
    if (assetData.keterangan !== undefined) {
      assetData.keterangan = assetData.keterangan.trim();
    }
    if (assetData.lokasi !== undefined) {
      assetData.lokasi = assetData.lokasi.trim();
    }
    if (assetData.asal_pengadaan !== undefined) {
      assetData.asal_pengadaan = assetData.asal_pengadaan.trim();
    }
  }
