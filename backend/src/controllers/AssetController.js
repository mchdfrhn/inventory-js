  ...
  async listAssets(req, res) {
    try {
      const {
        page,
        pageSize,
        limit, // Support both pageSize and limit for backward compatibility
        search,
        category_id,
        lokasi_id,
        status,
      ...
      // Use limit if pageSize is not provided (for backward compatibility)
      const finalPageSize = pageSize || limit || 10;

      if (page && finalPageSize) {
        const result = await this.assetUseCase.listAssetsPaginated(
          filter,
          parseInt(page),
          parseInt(finalPageSize),
        );
