...
func runMigration() {
	...
	dbname := getEnvOrDefault("DB_NAME", "inventaris")
	...
	// Step 0: Check and add the code column if it doesn't exist
	fmt.Println("Checking and adding code column if needed...")
	_, err = db.Exec(`
		DO $$
		BEGIN
			IF NOT EXISTS (
				...
				WHERE table_name = 'asset_categories' AND column_name = 'code'
			) THEN
	...
	`)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error checking/adding code column: %v\n", err)
		os.Exit(1)
	}
