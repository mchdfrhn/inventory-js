// This file should be run from the backend-nodejs directory
...
// Write test files
fs.writeFileSync('./test_categories.csv', categoryCSV);
fs.writeFileSync('./test_locations.csv', locationCSV);

console.log('Testing Category CSV Parsing...');
console.log('================================');

const categories = [];
fs.createReadStream('./test_categories.csv')
  .pipe(csv({
    skipEmptyLines: true,
    trim: true
  }))
  ...
  .on('end', () => {
    ...
    const locations = [];
    fs.createReadStream('./test_locations.csv')
      .pipe(csv({
        skipEmptyLines: true,
        trim: true
      }))
      ...
      .on('end', () => {
        ...
        // Clean up test files
        fs.unlinkSync('./test_categories.csv');
        fs.unlinkSync('./test_locations.csv');
